...
namespace
{
    ...
    void safe_export(char const *name, Fn &&fn)
    {
        try
        {
            fn();
        ...
        {
            throw std::runtime_error(std::string("pybind11 export failed in ") + name + ": " + e.what());
        }
        ...
        {
            throw std::runtime_error(std::string("pybind11 export failed in ") + name + ": unknown error");
        }
    }
...
{
    // Work around occasional CPython failures parsing generated __text_signature__
    // strings (seen as: "Internal error while parsing type signature").
    // This only affects Python-side introspection; it does not change behavior.
    py::options options;
    options.disable_function_signatures();

    safe_export("color", [&]()
                { export_color(m); });
    safe_export("composite_modes", [&]()
                { export_composite_modes(m); });
    safe_export("coord", [&]()
                { export_coord(m); });
    safe_export("envelope", [&]()
                { export_envelope(m); });
    safe_export("geometry", [&]()
                { export_geometry(m); });
    safe_export("gamma_method", [&]()
                { export_gamma_method(m); });
    safe_export("feature", [&]()
                { export_feature(m); });
    safe_export("featureset", [&]()
                { export_featureset(m); });
    safe_export("font_engine", [&]()
                { export_font_engine(m); });
    safe_export("fontset", [&]()
                { export_fontset(m); });
    safe_export("expression", [&]()
                { export_expression(m); });
    safe_export("datasource", [&]()
                { export_datasource(m); });
    safe_export("datasource_cache", [&]()
                { export_datasource_cache(m); });
#if defined(GRID_RENDERER)
    safe_export("grid", [&]()
                { export_grid(m); });
    safe_export("grid_view", [&]()
                { export_grid_view(m); });
#endif
    safe_export("image", [&]()
                { export_image(m); });
    safe_export("image_view", [&]()
                { export_image_view(m); });
    safe_export("layer", [&]()
                { export_layer(m); });
    safe_export("map", [&]()
                { export_map(m); });
    safe_export("projection", [&]()
                { export_projection(m); });
    safe_export("proj_transform", [&]()
                { export_proj_transform(m); });
    safe_export("query", [&]()
                { export_query(m); });
    safe_export("rule", [&]()
                { export_rule(m); });
    safe_export("symbolizer", [&]()
                { export_symbolizer(m); });
    safe_export("polygon_symbolizer", [&]()
                { export_polygon_symbolizer(m); });
    safe_export("line_symbolizer", [&]()
                { export_line_symbolizer(m); });
    safe_export("point_symbolizer", [&]()
                { export_point_symbolizer(m); });
    safe_export("style", [&]()
                { export_style(m); });
    safe_export("logger", [&]()
                { export_logger(m); });
    safe_export("placement_finder", [&]()
                { export_placement_finder(m); });
    safe_export("text_symbolizer", [&]()
                { export_text_symbolizer(m); });
    safe_export("palette", [&]()
                { export_palette(m); });
    safe_export("parameters", [&]()
                { export_parameters(m); });
    safe_export("debug_symbolizer", [&]()
                { export_debug_symbolizer(m); });
    safe_export("markers_symbolizer", [&]()
                { export_markers_symbolizer(m); });
    safe_export("polygon_pattern_symbolizer", [&]()
                { export_polygon_pattern_symbolizer(m); });
    safe_export("line_pattern_symbolizer", [&]()
                { export_line_pattern_symbolizer(m); });
    safe_export("raster_symbolizer", [&]()
                { export_raster_symbolizer(m); });
    safe_export("raster_colorizer", [&]()
                { export_raster_colorizer(m); });
    safe_export("scaling_method", [&]()
                { export_scaling_method(m); });
    safe_export("label_collision_detector", [&]()
                { export_label_collision_detector(m); });
    safe_export("dot_symbolizer", [&]()
                { export_dot_symbolizer(m); });
    safe_export("shield_symbolizer", [&]()
                { export_shield_symbolizer(m); });
    safe_export("group_symbolizer", [&]()
                { export_group_symbolizer(m); });
    safe_export("building_symbolizer", [&]()
